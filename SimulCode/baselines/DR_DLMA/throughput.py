...
def my_plot(file1, file2, file3):
	...
	for i in range(0, max_iter):
		if i < N:
			...
			aloha_temp_sum += aloha_reward[i]
			throughput_agent[0][i] = agent_temp_sum / (i+1)
			throughput_tdma[0][i] = tdma_temp_sum / (i+1)
			throughput_aloha[0][i] = aloha_temp_sum / (i+1)
		else:
			agent_temp_sum += agent_reward[i] - agent_reward[i-N]
			tdma_temp_sum += tdma_reward[i] - tdma_reward[i-N]
			aloha_temp_sum += aloha_reward[i] - aloha_reward[i-N]
			throughput_agent[0][i] = agent_temp_sum / N
			throughput_tdma[0][i] = tdma_temp_sum / N
			throughput_aloha[0][i] = aloha_temp_sum / N

	total_line, = plt.plot(throughput_agent[0]+throughput_tdma[0]+throughput_aloha[0], color='green', lw=1.2, label='total')
